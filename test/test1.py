...
# Giả sử file test.jpg nằm trong cùng thư mục với test1.py (E:\Convert_Image\test\)
...
def main():
    ...
    print("Bắt đầu tạo hiệu ứng edge_sketch từng bước...")
    ...
    print("Bước 2: Đã lưu ảnh biên tại step2_edges.jpg")
    
    # Bước 3: Đảo màu biên (sử dụng cv2.bitwise_not)
    edges_inv = cv2.bitwise_not(edges)
    cv2.imwrite('step3_inv.jpg', edges_inv)
    print("Bước 3: Đã lưu ảnh đảo màu biên tại step3_inv.jpg")
    
    # Bước 4: Chuyển sang BGR để tạo sketch cuối
    sketch = cv2.cvtColor(edges_inv, cv2.COLOR_GRAY2BGR)
    cv2.imwrite('step4_sketch.jpg', sketch)
    ...
    # cv2.imshow('Step 2: Edges', edges)
    # cv2.imshow('Step 3: Inverted Edges', edges_inv)
